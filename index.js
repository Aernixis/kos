...
async function updateListMessage() {
  ...
  try {
    ...
    const messages = [];
    const chunkSize = 1990; // split if > 2000 chars
    for (let i = 0; i < msgContent.length; i += chunkSize) {
      messages.push(msgContent.slice(i, i + chunkSize));
    }

    // If there is an old list message, try editing the first message and sending rest
    let firstMessage;
    if (data.listMessageId) {
      try {
        firstMessage = await channel.messages.fetch(data.listMessageId);
        if (firstMessage) {
          await firstMessage.edit(messages.shift()); // edit first message
        } else {
          firstMessage = await channel.send(messages.shift());
...
client.once("clientReady", () => {
  ...
  updateListMessage(); // background update on startup
});
